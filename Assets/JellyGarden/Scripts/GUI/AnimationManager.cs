...
public class AnimationManager : MonoBehaviour
{
    ...
    private bool NowIsActive, oneTime;
    private int[] Scores;
    private int temp = 0;

    void Start()
    {
        coun = FindObjectOfType<Counter_>();
    }
    void OnEnable()
    {
        if (PlayOnEnable)
        ...
        if (name == "MenuPlay")
        {
            ...
            //int stars = LevelManager.MyStarts[PlayerPrefs.GetInt("OpenLevel")];
    ...
    void Update()
    {
        ...
        {
            if (name == "MenuPlay" || name == "Settings" || name == "BoostInfo" || name == "LiveShop" || name == "BoostShop" || name == "Reward")
            { CloseMenu(); }
        ...
        if (NowIsActive)
        {
            ...
            if (time <= 1 && oneTime)
            {
                try
                {
                    ...
                    PlaysiveSDKUnity.instance.PostScore(LevelManager.TotalScore);
    ...
    void TargetCheck(bool check, int n)
    {
    ...
    public void BuyBoost(BoostType boostType, int price, int count)
    {
        ...
        if (LevelManager.THIS.SaveAllBestScores >= price)
        {
            int TempTotalScore = LevelManager.THIS.SaveAllBestScores - price;
            //PlayerPrefs.SetInt("TotalScore", TempTotalScore);
            InitScript.Instance.BuyBoost(boostType, price, count);
            LevelManager.THIS.SaveAllBestScores = TempTotalScore;
    ...
    public void WaitForDoneProcess(float timeout)
    {
        do
        {
