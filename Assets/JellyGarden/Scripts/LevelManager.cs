...
public class LevelManager : MonoBehaviour
{
    ...
    public static int[] ScoreNow = new int[100];
    public static int [] Scores = new int[100];
    public static int BestFaildScore;
    public int stars , SaveAllBestScores;
    ...
    public GameState gameStatus
    {
        ...
        set
        {
            ...
            else if (value == GameState.PreFailed)
            {
                ...
                if (Score > PlayerPrefs.GetInt("BestFaildScore" + currentLevel))
                {
                    BestFaildScore = Score;
                    PlayerPrefs.SetInt("BestFaildScore" + currentLevel, BestFaildScore);
                }else
                {
                    BestFaildScore = PlayerPrefs.GetInt("BestFaildScore" + currentLevel);
                }

                for (int i = 1 ; i < ScoreNow.Length; i++)
                {

                    if (ScoreNow[i] > Scores[i])
                    {
                        Scores[i] = ScoreNow[i];
                        PlayerPrefs.SetInt("Scores123c" + i, ScoreNow[i]);
                        PlayerPrefs.SetInt("Score18" + i, ScoreNow[i]);
                    }
                }
                TotalScore = SaveAllBestScores;
            ...
            else if (value == GameState.Win)
            {
                ...
                {                 
                    SaveAllBestScores += Score;
                    ScoreNow[currentLevel] = Score;
                    PlayerPrefs.SetInt("Temp" , SaveAllBestScores);
                }
    ...
    void Start()
    {
        ...
        SaveAllBestScores = TotalScore;
        LevelsMap tmpLevelsMap = LevelsMap.GetComponent<LevelsMap>();
        IMapProgressManager _mapProgressManager = new PlayerPrefsMapProgressManager();
        List<MapLevel> tmpMapLevels = tmpLevelsMap.GetMapLevels();
        for (int i = 1; i < 100; i++)
        {
            if (PlayerPrefs.GetInt("Scores123c" + i) != 0)
            {
                int temp = PlayerPrefs.GetInt("Scores123c" + i);
                Scores[i] = temp;
                ScoreNow[i] = temp;
            }
            else
            {
                if (tmpMapLevels[i-1].StarsCount != 0)
                {
                    _mapProgressManager.SaveLevelStarsCount(i, 0);              
                }
            }
        }
#if FACEBOOK
